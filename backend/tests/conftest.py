...
def _decode_container_logs(log_payload: object) -> str:
    ...
    if isinstance(log_payload, tuple):
        return "".join(_decode_container_logs(chunk) for chunk in log_payload)
...
def _wait_for_container_log(
    ...
    *,
    ...
    while True:
        latest_logs = _decode_container_logs(container.get_logs())
        if expected_substring in latest_logs:
            return

        if time.monotonic() >= deadline:
            pytest.fail(
                "Timed out waiting for substring "
                f"{expected_substring!r} in container logs.\n"
