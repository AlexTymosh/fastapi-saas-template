...
def test_drain_helper_claims_then_processes_events(
    migrated_session_factory,
    monkeypatch,
) -> None:
    from tests.helpers.outbox import drain_outbox
    ...
    run_async(_seed())
    drain_outbox(migrated_session_factory, monkeypatch)
    assert len(sent_event_ids) == 1


def test_pending_event_is_not_processed_directly(
    migrated_session_factory,
    monkeypatch,
) -> None:
    sink = InMemoryInviteTokenSink()
    ...
    async def _run() -> None:
        async with migrated_session_factory() as session:
            event = OutboxEvent(
                event_type=OutboxEventType.INVITE_CREATED.value,
                aggregate_type="invite",
                aggregate_id=UUID("00000000-0000-0000-0000-000000000311"),
                payload_json={
                    "invite_id": "00000000-0000-0000-0000-000000000311",
                    "raw_token": "secret-token",
                },
                status=OutboxStatus.PENDING.value,
            )
...
def test_worker_runtime_fails_without_redis_url(monkeypatch) -> None:
    monkeypatch.delenv("REDIS__URL", raising=False)
    from app.core.config.settings import get_settings

    get_settings.cache_clear()
    from app.outbox.worker_runtime import bootstrap_worker_runtime

    try:
        bootstrap_worker_runtime()
    except RuntimeError as exc:
        assert "REDIS__URL" in str(exc)
    else:
        raise AssertionError("Expected RuntimeError when REDIS__URL is missing")
    finally:
        get_settings.cache_clear()
