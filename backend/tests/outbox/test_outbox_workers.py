...
def test_worker_runtime_fails_without_redis(monkeypatch) -> None:
    from app.core.tasks import broker as broker_module
    ...
    monkeypatch.setattr(broker_module, "_configured_broker", None)

    with pytest.raises(RuntimeError, match="REDIS__URL"):
        broker_module.configure_broker(require_redis=True)
