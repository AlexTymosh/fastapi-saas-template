...
@pytest.mark.security
@pytest.mark.rate_limit
def test_settings_parses_rate_limit_policy_override(monkeypatch) -> None:
...
@pytest.mark.parametrize(
    ("env_name", "env_value"),
    [
        ("RATE_LIMITING__POLICIES__TENANT_WRITE__LIMIT", "0"),
        ("RATE_LIMITING__POLICIES__TENANT_WRITE__WINDOW_SECONDS", "0"),
    ],
)
def test_settings_rejects_invalid_rate_limit_policy_override_values(
    monkeypatch,
    env_name: str,
    env_value: str,
) -> None:
...
@pytest.mark.security
@pytest.mark.rate_limit
def test_settings_rejects_relaxed_rate_limit_mode_in_prod(monkeypatch) -> None:
    monkeypatch.setenv("APP__ENVIRONMENT", "prod")
    monkeypatch.setenv("AUTH__ENABLED", "true")
    monkeypatch.setenv("API__DOCS_ENABLED", "false")
    monkeypatch.setenv("REQUEST_CONTEXT__TRUST_INCOMING_REQUEST_ID", "false")
...
@pytest.mark.security
@pytest.mark.rate_limit
def test_settings_accepts_panic_rate_limit_mode_in_prod(monkeypatch) -> None:
    monkeypatch.setenv("APP__ENVIRONMENT", "prod")
    monkeypatch.setenv("AUTH__ENABLED", "true")
    monkeypatch.setenv("API__DOCS_ENABLED", "false")
    monkeypatch.setenv("REQUEST_CONTEXT__TRUST_INCOMING_REQUEST_ID", "false")
