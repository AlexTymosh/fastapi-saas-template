...
def test_platform_staff_creation(staff_env, migrated_session_factory):
    ...
    assert create_response.headers["location"].endswith(
        f"/api/v1/platform/staff/{create_response.json()['id']}"
    )
    get_response = staff_env.admin_bundle.client.get(
...
def test_platform_staff_lifecycle_management(staff_env):
    ...
    # Verify idempotent redundant restores
    assert (
        staff_env.admin_bundle.client.post(
            f"/api/v1/platform/staff/{staff_env.support_staff.id}/restore",
            json={"reason": "again"},
...
def test_platform_staff_service_suspend_already_suspended_is_idempotent_with_no_audit(
    migrated_session_factory,
) -> None:
    actor_user, actor_staff = _seed_staff(
        migrated_session_factory,
        ext_id="kc-service-suspend-idem-actor",
        email="service-suspend-idem-actor@example.com",
        role=PlatformRole.PLATFORM_ADMIN.value,
    )
    _, target_staff = _seed_staff(
        migrated_session_factory,
        ext_id="kc-service-suspend-idem-target",
        email="service-suspend-idem-target@example.com",
        role=PlatformRole.SUPPORT_AGENT.value,
        status=PlatformStaffStatus.SUSPENDED,
    )
    ...
    async def _run():
        async with migrated_session_factory() as session:
            async with session.begin():
                ...
                assert staff is not None
                staff.suspended_reason = "original"
            ...
            async with session.begin():
                ...
                assert staff.status == PlatformStaffStatus.SUSPENDED.value
                assert staff.suspended_reason == "original"
...
def test_platform_staff_service_restore_already_active_is_idempotent_with_no_audit(
    migrated_session_factory,
) -> None:
    actor_user, _ = _seed_staff(
        migrated_session_factory,
