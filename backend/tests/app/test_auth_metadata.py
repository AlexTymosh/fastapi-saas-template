...
def test_warn_mode_logs_and_does_not_raise(caplog) -> None:
    fetch, _ = _fetcher(discovery={"issuer": "https://other", "jwks_uri": JWKS_URL})

    run_async(
        init_auth_validation(
            _settings(metadata_validation="warn"),
            fetch_json=fetch,
        )
    )

    assert "auth_metadata_validation_warning" in caplog.text
...
def test_disabled_mode_does_not_fetch_metadata() -> None:
    ...
    run_async(
        init_auth_validation(
            _settings(metadata_validation="disabled"),
            fetch_json=_fetch,
        )
    )
