...
def test_create_app_uses_custom_request_id_header_end_to_end(monkeypatch) -> None:
    ...
    problem_response = client.get("/missing-route")

    assert problem_response.status_code == 404
    assert problem_response.headers["content-type"].startswith(
        "application/problem+json"
    )
    assert "x-correlation-id" in problem_response.headers
    assert "x-request-id" not in problem_response.headers

    body = problem_response.json()
    assert "request_id" in body
    assert problem_response.headers["x-correlation-id"] == body["request_id"]
