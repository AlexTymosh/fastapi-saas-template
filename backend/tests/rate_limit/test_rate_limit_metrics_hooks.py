...
def test_rate_limit_dependency_records_duration_for_all_outcomes(
    ...
    client = _build_app(monkeypatch, runtime=runtime, fail_open=fail_open)
    with client as api_client:
        response = api_client.get("/api/v1/test/rate-limit/protected")

    assert response.status_code == expected_status
    assert decisions == [expected_result]
    assert [payload["result"] for payload in durations] == [expected_result]

    payload = durations[0]
    assert payload["policy_name"] == "test_probe"
    assert payload["identifier_kind"] == "user"
