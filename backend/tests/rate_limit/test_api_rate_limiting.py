...
# Invite create/resend business-scope buckets are now enforced inside InviteService
# after tenant authorization. The old endpoint tests that required these
# buckets to run before DB access encoded the cross-tenant quota poisoning
# bug fixed by this change. Actor-level endpoint throttling remains covered
# above; post-authorization business buckets are covered by
# test_invite_rate_limit_contexts.py.
