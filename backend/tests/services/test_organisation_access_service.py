...
def test_get_organisation_for_member_uses_provisioning_and_access_checks() -> None:
    ...
    service.user_service.provision_current_user.assert_awaited_once_with(identity)
...
def test_list_memberships_for_member_organisation_uses_access_use_case() -> None:
    service = OrganisationAccessService(session=AsyncMock())
    identity = _identity()
    organisation_id = uuid4()
    user = User(
    ...
    memberships = [Membership(user_id=uuid4(), organisation_id=organisation_id)]
