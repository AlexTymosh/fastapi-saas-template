...
def test_accept_invite_rejects_expired_invite_and_marks_expired() -> None:
    service = _service()
    service.invite_repository = AsyncMock()
    service.invite_repository.get_by_token_hash = AsyncMock(
        return_value=Invite(
    ...
    service.invite_repository.mark_status.assert_awaited_once_with(
        service.invite_repository.get_by_token_hash.return_value,
        InviteStatus.EXPIRED,
    )


def test_accept_invite_accepts_valid_unexpired_pending_invite() -> None:
    service = _service()
    invite = Invite(
        email="invited@example.com",
        organisation_id=uuid4(),
