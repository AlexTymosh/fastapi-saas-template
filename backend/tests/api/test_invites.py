...
def test_invite_accept_returns_conflict_for_user_with_active_membership(
    ...
    with invitee_client as client:
        ...
        assert accepted.json()["status"] == 409
        assert accepted.json()["error_code"] == "conflict_error"
    ...
    async def _assert_membership_not_transferred() -> None:
        async with migrated_session_factory() as session:
            ...
            assert str(memberships[0].organisation_id) == source_org_id
            ...
            transfer_invite = invite_result.scalar_one()
            assert transfer_invite.status == InviteStatus.PENDING
