...
def test_create_first_organisation_succeeds(
    ...
    with test_client_bundle.client as client:
        response = client.post(
            "/api/v1/organisations",
            json={"name": "First Organisation", "slug": "first-organisation"},
        )

    assert response.status_code == 201


def test_create_second_active_organisation_for_same_user_returns_conflict(
    authenticated_client_factory,
    migrated_database_url: str,
) -> None:
    identity = _identity_for(
        external_auth_id="kc-second-org-user",
    ...
    with test_client_bundle.client as client:
        first = client.post(
            "/api/v1/organisations",
            json={"name": "Second Org First", "slug": "second-org-first"},
        )
        second = client.post(
            "/api/v1/organisations",
            json={"name": "Second Org Next", "slug": "second-org-next"},
        )
...
def test_users_me_returns_single_membership_contract(
    ...
    with test_client_bundle.client as client:
        create_org = client.post(
            "/api/v1/organisations",
            json={"name": "Users Me Contract Org", "slug": "users-me-contract-org"},
        )
