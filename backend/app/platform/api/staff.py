...
async def list_platform_staff(
    ...
    rows, total = await PlatformStaffService(db_session).list_staff(
        limit=limit, offset=offset
    )
    return PlatformStaffCollectionResponse(
        data=[PlatformStaffResponse.model_validate(row) for row in rows],
...
async def create_platform_staff(
    ...
    staff = await PlatformStaffService(write_context.session).create_platform_staff(
...
async def update_platform_staff_role(
    ...
    actor = write_context.actor
    staff = await PlatformStaffService(write_context.session).update_staff_role(
        staff_id=staff_id,
        role=payload.role,
        actor=actor,
        reason=payload.reason,
