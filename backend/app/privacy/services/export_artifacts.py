...
from app.privacy.models.data_subject_request import DataSubjectRequestStatus, DataSubjectRequestType
from app.privacy.models.export_artifact import ExportArtifact, ExportArtifactFormat, ExportArtifactStatus, ExportArtifactStorageBackend
...
class ExportArtifactService:
    def __init__(self, session: AsyncSession) -> None:
        ...
        self.storage = LocalStorageAdapter(self.settings.privacy_exports.local_storage_path)

    async def request_export_artifact(self, *, request_id: UUID, requested_by_user_id: UUID, audit_context: AuditContext) -> ExportArtifact:
        dsr = await self.dsr_repo.get_by_id(request_id)
        if dsr is None:
            raise NotFoundError(detail="Data subject request not found")
        if dsr.request_type != DataSubjectRequestType.EXPORT.value or dsr.status != DataSubjectRequestStatus.APPROVED.value:
            raise ConflictError(detail="Export artifact can only be requested for approved export data-subject requests")
        artifact = await self.repo.create(
            ...
            expires_at=datetime.now(UTC) + timedelta(days=self.settings.privacy_exports.artifact_retention_days),
        )
        await self.audit.record_event(audit_context=audit_context, category=AuditCategory.COMPLIANCE, action=AuditAction.EXPORT_ARTIFACT_REQUESTED, target_type=AuditTargetType.EXPORT_ARTIFACT, target_id=artifact.id, metadata_json={"status": artifact.status, "format": artifact.format})
        return artifact
