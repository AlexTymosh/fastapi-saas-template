...
Platform-роли не являются tenant-ролями. Backend авторизует platform-доступ только по таблице `platform_staff`; JWT roles не должны выдавать request-time platform authorization и могут в будущем использоваться только как controlled JIT provisioning input для локальных DB records. Platform actors работают только через `/api/v1/platform/*` и не получают bypass для `/api/v1/organisations/*`.
