...
The current foundation prepares rate-limit metrics through no-op OpenTelemetry API instruments. This keeps metric hooks in place without changing runtime behavior until a concrete SDK and exporter are introduced.
...
Allowed rate-limit result values:
...
If route template is unavailable, use `"unknown"`.
