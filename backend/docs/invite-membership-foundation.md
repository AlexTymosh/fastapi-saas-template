...
Outbox workers now use DB-backed status/attempt tracking as the source of truth and do not rely on Dramatiq retries for business delivery retries. A dedicated dispatcher process claims due events and enqueues them to Dramatiq workers. Lifecycle: `pending -> processing -> processed`, or `pending -> processing -> pending/failed` on delivery error. External delivery is executed outside DB transactions.
...
- Worker: `dramatiq app.outbox.workers`
- Dispatcher: `python -m app.outbox.dispatcher --interval 5 --batch-size 100`
- In Docker Compose, run `app`, `worker`, and `outbox-dispatcher` together with Redis and PostgreSQL.

Known limitation (P0): if DB claim commits but queue enqueue fails, events may remain in `processing` until a future stale-lock recovery flow is implemented.
