...
### E2E
- Full API flow tests through HTTP client.
...
Notes:
- `external_db` requires explicit CLI flag.
- `external_db` also requires `ENABLE_EXTERNAL_MIGRATION_DB_TEST=1`.
...
- For auth-heavy flows, use a documented test auth strategy or test Keycloak realm; do not silently bypass security without documenting why.
