...
| Race condition в admin/staff операциях | Удалён последний platform admin, нарушен owner invariant | membership/platform staff |
...
| PATCH | `/api/v1/platform/staff/{staff_id}/role` | Да | PLATFORM_STAFF_MANAGE | Global platform | Нет | Да | Средний/Высокий: риск race condition |
| POST | `/api/v1/platform/staff/{staff_id}/suspend` | Да | PLATFORM_STAFF_MANAGE | Global platform | Нет | Да | Средний/Высокий: риск race condition |
...
| Last platform admin race | Вероятно отсутствует | Concurrent demote/suspend test | Platform lockout | P1 |
