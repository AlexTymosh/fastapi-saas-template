...
| Tenant isolation в основном реализована корректно | Organisation/membership services + `backend/tests/api/test_tenant_bola_idor_regressions.py` | Исправлено | Пользователь передаёт чужой `organisation_id` / `membership_id` / `invite_id`; проверки membership и загрузка ресурса в границах организации блокируют доступ | Сохранить паттерн и BOLA/IDOR regression tests для tenant UUID path params | Fixed |
...
|---|---|---|---|---|---|
| `GET /api/v1/organisations/{organisation_id}` | organisation_id | `OrganisationAccessService`: user active + org active + membership | Низкий | Fixed: regression test подтверждает, что пользователь org A не читает org B | Fixed |
| `PATCH /api/v1/organisations/{organisation_id}` | organisation_id | Сервис проверяет membership owner/admin актора | Низкий | Fixed: regression test подтверждает блокировку update чужой org и неизменность БД | Fixed |
| `DELETE /api/v1/organisations/{organisation_id}` | organisation_id | Только owner | Низкий | Fixed: regression test подтверждает запрет удаления чужой org для admin/member/non-member и неизменность БД | Fixed |
| `GET /api/v1/organisations/{organisation_id}/directory` | organisation_id | Актор должен быть активным членом | Низкий | Fixed: cross-org directory regression test добавлен | Fixed |
| `GET /api/v1/organisations/{organisation_id}/memberships` | organisation_id | Только owner/admin | Низкий | Fixed: member получает Problem Details `403` | Fixed |
| `PATCH /api/v1/organisations/{organisation_id}/memberships/{membership_id}/role` | membership_id | Целевой membership загружается по membership_id + organisation_id | Низкий | Fixed: cross-org `membership_id` возвращает Problem Details `404`, роль не меняется | Fixed |
| `DELETE /api/v1/organisations/{organisation_id}/memberships/{membership_id}` | membership_id | Ограничен org + правилами owner/admin | Низкий | Fixed: cross-org `membership_id` возвращает Problem Details `404`, membership остаётся active | Fixed |
| `POST /api/v1/organisations/{organisation_id}/invites` | organisation_id | Актор должен быть owner/admin org | Низкий | Fixed: пользователь org A не создаёт invite в org B, invite не появляется в БД | Fixed |
| `DELETE /api/v1/organisations/{organisation_id}/invites/{invite_id}` | invite_id | Invite загружается по invite_id + organisation_id | Низкий | Fixed: cross-org `invite_id` возвращает Problem Details `404`, invite остаётся pending | Fixed |
| `POST /api/v1/organisations/{organisation_id}/invites/{invite_id}/resend` | invite_id | Invite загружается по invite_id + organisation_id | Низкий | Fixed: cross-org `invite_id` возвращает Problem Details `404`, invite token не ротируется | Fixed |
...
| Tenant isolation | Покрыто `backend/tests/api/test_tenant_bola_idor_regressions.py` для organisation_id, membership_id, invite_id | Поддерживать regression tests при добавлении tenant endpoints | BOLA/IDOR | Fixed |
...
| 4 | Fixed: добавить BOLA regression tests | `backend/tests/api/test_tenant_bola_idor_regressions.py` | Cross-tenant UUID доступ зафиксирован как заблокированный; чужие ресурсы не изменяются в БД | Fixed |
