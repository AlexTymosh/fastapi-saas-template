...
| Fixed: limited platform audit view реализован | `permissions.py`, `platform/api/audit_events.py`, `platform/schemas/platform_audit_events.py` | `AUDIT_READ_LIMITED` применяется backend endpoint/schema-level redaction | Support/compliance triage видит безопасный audit summary без raw metadata/IP/user-agent/reason/actor id | Сохранять regression tests для full/limited/no-permission/filtering контрактов | Fixed |
...
| Fixed: отзыв invite имеет rate limit | `invites/api/invites.py` | Риск перебора invite ID / злоупотребления скомпрометированным admin снижен | Admin токен используется для brute-force invite ID внутри org | `invite_mutation` fail-closed policy + endpoint coverage/429 tests | Fixed |
...
| Logging redaction усилен | `logging/processors.py`, `tests/logging/test_processors.py` | Низкий/контролируемый | Fixed: ключи нормализуются (`-`, `.`, camel/PascalCase), sensitive markers редактируются рекурсивно; Bearer/Basic/JWT-like значения полностью redacted; email masking сохранён | Fixed |
...
|---|---|---|---|---|---|
| Logging | Fixed: redaction покрывает вариантные sensitive keys | `logging/processors.py`, `tests/logging/test_processors.py` | Низкий/контролируемый | Сохранить централизованную политику sensitive-key detection и value-based redaction для Bearer/Basic/JWT-like строк | Fixed |
...
| No secret logging | Fixed for structured logging processor | Варианты redaction: exact keys, raw-token, raw_token, clientSecret, x-api-key, encrypted_raw_token, set-cookie, authorizationHeader, Bearer/Basic/JWT-like values, email masking | Риск утечки через application/security structured logs снижен; сохранять regression tests | Fixed |
...
| P3 | Rate limit | Fixed: invite revoke покрыт `invite_mutation` fail-closed policy | `invites/api/invites.py`, `core/rate_limit/policies.py` | Злоупотребление/перебор с admin токеном ограничены 429/503 contract | Сохранить endpoint coverage и API regression tests |
...
| `backend/app/core/rate_limit/policies.py` | Fixed: добавлена полная матрица authenticated/tenant/organisation/invite/platform/audit policies, включая `platform_write` и `platform_staff_write` | Sensitive authenticated endpoint groups зарегистрированы; write/audit/platform policies fail-closed, low-risk reads fail-open | `tests/rate_limit/test_policy_registry.py`, `tests/rate_limit/test_endpoint_protection.py`, `tests/api/test_rate_limiting.py`, `tests/platform/test_platform_write_rate_limiting.py` |
